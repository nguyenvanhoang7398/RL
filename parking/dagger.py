...
environment = construct_task2_env(tensor_state=False)
...
class DAGGER(object):
    ...
    """
    def __init__(self, n_lanes, n_width, policy_net, env, device, rsm):
    ...
    def randomPolicy(self, state, env):
        ...
        while not state.isDone():
            ...
            reward = reward_shape(curr_state, next_state, reward, False, self.reward_shaping_mtx)
            reward += state.getReward()
        return reward
    ...
    def init_mcts(self, explorationParam=1., random_seed=random_seed):
        ...
        return MonteCarloTreeSearch(env=self.env, numiters=numiters, explorationParam=explorationParam,
                                    playoutPolicy=self.randomPolicy, random_seed=random_seed)
...
def run_dagger(policy_net=0, env=0, rs=0):
    ...
    if rs != 0:
        reward_shaping_path = rs
        print(reward_shaping_path)
        if os.path.exists(reward_shaping_path):
            print("Use reward shaping")
            reward_shaping_mtx_internal = load_from_pickle(reward_shaping_path)
            use_reward_shaping = True
        else:
            print("Do not use reward shaping")
            reward_shaping_mtx_internal = np.zeros(n_width, n_lanes)
            use_reward_shaping = False
    else:
        reward_shaping_mtx_internal = reward_shaping_mtx
    dagger = DAGGER(n_lanes, n_width, policy_net, env, device, reward_shaping_mtx_internal)
    return dagger.run()
